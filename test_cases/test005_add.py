...
mylog=Mylog('add_pro').mylog
Cookies=None
header=None
api_data=DoExcel(constant.data_xlsx).read_excel('add_pro')
...
class ApiTest(unittest.TestCase):
    try:
        ...
        def test_api(self,case):
            ...
            DoExcel(constant.data_xlsx).write_excel('add_pro',case.case_id + 1, 7, actual)
            result = None
            try:
                self.assertEqual(case.expect,actual)
                result='pass'
                print('第{}条测试用例：[{}]执行结果为：{}'.format(case.case_id,case.title,result))
                DoExcel(constant.data_xlsx).write_excel('add_pro',case.case_id+1,8,result)
